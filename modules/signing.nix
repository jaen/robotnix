...
in
{
  ...
  in {
    ...
    build.signing.withKeys = keysDir: script: 
      let
        # Workaround for https://github.com/NixOS/nix/pull/9095
        keyPath = "${builtins.path {path = config.signing.sopsDecrypt.key; name = "sops-decrypt.key";} }";
        sopsConfigPath = "${builtins.path {path = config.signing.sopsDecrypt.sopsConfig; name = "sops.yaml";} }";
      in
        ''
          set -x
          ...
          if [[ "$KEYSDIR" ]]; then
            ...
            # copy the keys over
            export SOPS_AGE_KEY_FILE=${lib.optionalString (config.signing.sopsDecrypt.enable && config.signing.sopsDecrypt.keyType == "age") keyPath};
            export SOPS_PGP_FP=${lib.optionalString (config.signing.sopsDecrypt.enable && config.signing.sopsDecrypt.keyType == "pgp") keyPath};
            ...
            set -x
            TMPDIR=$(mktemp -d)
            ${lib.optionalString config.signing.sopsDecrypt.enable "ln -s ${sopsConfigPath} $TMPDIR/.sops.yaml"}
